...
class AsyncChunkManagerForAR:
    def __init__(self, connector, stage_id, model=None):
        ...
        self.first_chunk_for_generation: dict[str, bool] = defaultdict(lambda: True)
        self.track_if_upstream_finished: dict[str, bool] = defaultdict(bool)
        self.accumulate_chunks_for_generation: dict[str, list] = defaultdict(list)
        self.model = model

    def _prepare_connector_key(self, chunk_id, stage_id, request_id):
        return f"{request_id}_{stage_id}_{chunk_id}"

    def _process_chunk_for_ar(self, pooler_output, request, next_stage_chunk_process_input_func):
    ...
    def _process_chunk_for_generation(self, pooler_output, request, next_stage_chunk_process_input_func):
        if not next_stage_chunk_process_input_func:
            return

        token_ids = []

        if len(pooler_output) > 0:
            self.accumulate_chunks_for_generation[request.request_id].extend(pooler_output)
            if len(self.accumulate_chunks_for_generation[request.request_id]) == 36 or request.is_finished():
                if not self.first_chunk_for_generation[request.request_id] or request.is_finished():
                    token_ids = self.accumulate_chunks_for_generation[request.request_id]
                    self.send_chunk(
                        token_ids, request, next_stage_chunk_process_input_func, last_chunk=request.is_finished()
                    )
                else:
                    self.first_chunk_for_generation[request.request_id] = False

                self.accumulate_chunks_for_generation[request.request_id] = []

    def process_chunk(self, pooler_output, request, next_stage_chunk_process_input_func, generation=False):
        if generation:
            self._process_chunk_for_generation(pooler_output, request, next_stage_chunk_process_input_func)
        else:
            self._process_chunk_for_ar(pooler_output, request, next_stage_chunk_process_input_func)
    ...
    def _retrieve_chunk_from_connector(self, request_id, validate_chunk_func=None):
    ...
    def receive_chunk(self, active_requests: list[OmniRequest], validate_chunk_func: Callable | None = None) -> None:
        ...
        for request in active_requests:
            ...
            chunk = self._retrieve_chunk_from_connector(req_id, validate_chunk_func)
...
class AsyncChunkManagerForGeneration:
    def __init__(self, connector, stage_id):
        self.connector = connector
        self.processed_chunks: dict[str, int] = defaultdict(int)
        self.received_chunks: dict[str, int] = defaultdict(int)
        self.stage_id = stage_id
    ...
    def receive_chunk(self, active_requests: list[OmniRequest], validate_chunk_func: Callable | None = None) -> None:
        ...
        for request in active_requests:
            ...
            if chunk:
                # Update request with new chunk tokens
                request.prompt_token_ids = chunk["tokens"]
                # Reset state so request is treated as fresh with new tokens
