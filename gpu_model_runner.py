...
from vllm.config import CompilationLevel, CUDAGraphMode, VllmConfig, get_layers_from_vllm_config, update_config
...
class GPUModelRunner(LoRAModelRunnerMixin, KVConnectorModelRunnerMixin):
    def __init__(
        ...
        self.async_output_copy_stream = torch.cuda.Stream() if self.use_async_scheduling else None
        ...
        self.sampled_token_ids_pinned_cpu = torch.empty(
            (self.max_model_len, 1), dtype=torch.int64, device="cpu", pin_memory=self.pin_memory
        )
    ...
    def _may_reorder_batch(self, scheduler_output: "SchedulerOutput") -> None:
        ...
        if self.reorder_batch_threshold is not None:
            ...
            reorder_batch_to_split_decodes_and_prefills(
                self.input_batch, scheduler_output, decode_threshold=self.reorder_batch_threshold
            )
        ...
        num_accepted_tokens = (
            (
                torch.cat(
                    [
                        output_token_ids,
                        torch.full((output_token_ids.size(0), 1), -1, device=output_token_ids.device),
                    ],
                    dim=1,
                )
                == -1
    ...
    def _prepare_inputs(
        ...
        print(f"Commit block table for {num_reqs} requests")
        ...
        # Only relevant for models using M-RoPE (e.g, Qwen2-VL)
        print(f"uses_mrope_positions: {self.uses_mrope}")
        if self.uses_mrope:
        ...
        print(f"self.input_batch.req_prompt_embeds: {self.input_batch.req_prompt_embeds}")
        if self.input_batch.req_prompt_embeds:
            ...
            for req_idx in range(num_reqs):
                ...
                output_idx += num_sched
        print("Compute slot mapping")
        self.input_batch.block_table.compute_slot_mapping(req_indices, positions_np)
        print("commit slot mapping")
        ...
        print(f"Use spec decode: {use_spec_decode}")
        if not use_spec_decode:
            ...
            print(f"num_decode_draft_tokens init: {num_decode_draft_tokens}")
            for req_id, draft_token_ids in scheduler_output.scheduled_spec_decode_tokens.items():
                print(f"Spec decode tokens for req {req_id}: {draft_token_ids}")
            ...
            print(f"Spec decode metadata: {spec_decode_metadata}")
        ...
        print("Prepare kv sharing fast prefill")
        ...
        for kv_cache_group_id, kv_cache_group_spec in enumerate(self.kv_cache_config.kv_cache_groups):
            ...
            print(f"Common attn metadata: {common_attn_metadata}")
            print(f"self.speculative_config : {self.speculative_config}")
            ...
            for attn_group in self.attn_groups[kv_cache_group_id]:
                print(f"Prepare attn metadata for attn group: {attn_group}")
                # Prepare for cascade attention if enabled & beneficial.
                common_prefix_len = 0
                builder = attn_group.get_metadata_builder()
                print(f"Metadata builder: {builder}")
                print(f"cascade_attn_enabled: {self.cascade_attn_enabled}")
                ...
                print(f"use_spec_decode: {use_spec_decode}, builder: {builder}")
                ...
                if ubatch_slices is not None:
                    ...
                    for ubid, common_attn_metadata in enumerate(common_attn_metadata_list):
                        attn_metadata_i = attn_group.get_metadata_builder(ubatch_id=ubid).build(
                            common_prefix_len=common_prefix_len, common_attn_metadata=common_attn_metadata
                        )
                    ...
                    print(f"build finished: {attn_metadata_i}")
                    for layer_name in attn_group.layer_names:
                        attn_metadata[layer_name] = attn_metadata_i
                    print("ELSE FINISHED")
        print(f"self.lora_config: {self.lora_config}")
        # Hot-Swap lora model
        if self.lora_config:
        ...
        for modality, num_items, mm_kwargs_group in group_mm_kwargs_by_modality(
            ...
            if self.is_multimodal_pruning_enabled and modality == "video":
                ...
                for i in range(0, num_items, micro_batch_size):
                    micro_batch_mm_inputs = dict((k, v[i : i + micro_batch_size]) for k, v in mm_kwargs_group.items())
    ...
    def _gather_mm_embeddings(
        ...
        if should_sync_mrope_positions:
            self._calc_mrope_positions(scheduler_output)
            self.mrope_positions.copy_to_gpu(scheduler_output.total_num_scheduled_tokens)
    ...
    def get_supported_pooling_tasks(self) -> list[PoolingTask]:
        ...
        if self.scheduler_config.chunked_prefill_enabled and "encode" in supported_tasks:
    ...
    def execute_model(
        ...
        print("INTO EXECUTE MODEL")
        with record_function_or_nullcontext("Preprocess"):
            with self.synchronize_input_prep():
                # Update persistent batch states.
                self._update_states(scheduler_output)
                print("Updated states finished.")
                ...
                print(f"_prepare_inputs called with scheduler_output: {scheduler_output}")
                ...
                print("Prepared inputs finished.")
            print(f"Max query len: {max_query_len}")
            (
                num_scheduled_tokens,
                num_input_tokens,
        ...
        print(f"Model run begin: num_input_tokens={num_input_tokens}, num_scheduled_tokens={num_scheduled_tokens}")
        # Run the model.
            ...
            if not self.broadcast_pp_output:
                ...
                if not get_pp_group().is_last_rank:
                    ...
                    get_pp_group().send_tensor_dict(
                        hidden_states.tensors, all_gather_group=get_tp_group(), all_gather_tensors=all_gather_tensors
                    )
                    logits = None
                else:
                    sample_hidden_states = hidden_states[logits_indices]
        ...
        with record_function_or_nullcontext("Bookkeep"):
            (
                ...
                req_id_to_index_output_copy,
                invalid_req_indices,
            ) = self._bookkeeping_sync(scheduler_output, sampler_output, logits, hidden_states, num_scheduled_tokens)
        ...
        if not randomize_inputs:
            ...
            def rand_input_ids() -> torch.Tensor:
                return torch.randint_like(
                    self.input_ids.gpu, low=0, high=self.model_config.get_vocab_size(), dtype=input_ids.dtype
                )
        ...
        with self.maybe_dummy_run_with_lora(self.lora_config, num_scheduled_tokens, remove_lora):
            ...
            _cg_mode, batch_descriptor = (
                self.cudagraph_dispatcher.dispatch(
                    BatchDescriptor(num_tokens=num_tokens_after_padding, uniform_decode=uniform_decode)
                )
                if not is_profile
        ...
        with freeze_gc(), graph_capture(device=self.device):
            ...
            if cudagraph_mode.mixed_mode() != CUDAGraphMode.NONE:
                ...
                self._capture_cudagraphs(
                    compilation_cases, cudagraph_runtime_mode=cudagraph_runtime_mode, uniform_decode=False
                )
            ...
            if cudagraph_mode.decode_mode() == CUDAGraphMode.FULL and cudagraph_mode.separate_routine():
                ...
                decode_cudagraph_batch_sizes = [
                    x for x in self.cudagraph_batch_sizes if x <= max_num_tokens and x >= self.uniform_decode_query_len
                ]
    ...
    def _capture_cudagraphs(
        self, compilation_cases: list[int], cudagraph_runtime_mode: CUDAGraphMode, uniform_decode: bool
    ):
        assert cudagraph_runtime_mode != CUDAGraphMode.NONE and cudagraph_runtime_mode in [
            CUDAGraphMode.FULL,
            CUDAGraphMode.PIECEWISE,
        ]
        ...
        if is_global_first_rank():
            compilation_cases = tqdm(
                ...
                desc="Capturing CUDA graphs ({}, {})".format(
                    "decode" if uniform_decode else "mixed prefill-decode", cudagraph_runtime_mode.name
                ),
            )
    ...
    def initialize_cudagraph_capture(self) -> None:
        ...
        if cudagraph_mode.mixed_mode() == CUDAGraphMode.FULL and min_cg_support != AttentionCGSupport.ALWAYS:
            ...
            if min_cg_support == AttentionCGSupport.NEVER:
                ...
                msg += "; please try cudagraph_mode=PIECEWISE, and make sure compilation level is piecewise"
            ...
            for layer_name in group.layer_names:
                ...
                if isinstance(kv_cache_spec, AttentionSpec) and kv_cache.shape[0] == 2:
                    ...
                    kv_cache.as_strided_(
                        size=kv_cache.shape, stride=(hidden_size, 2 * hidden_size, *kv_cache.stride()[2:])
                    )
    ...
    def initialize_kv_cache(self, kv_cache_config: KVCacheConfig) -> None:
        ...
        if self.dcp_world_size > 1:
            ...
            layers = get_layers_from_vllm_config(self.vllm_config, AttentionLayerBase, layer_names)
    ...
    def get_kv_cache_spec(self) -> dict[str, KVCacheSpec]:
        ...
        ds_indexer_layers = get_layers_from_vllm_config(self.vllm_config, DeepseekV32IndexerCache)
